        ...
        <template v-for="(item, index) of currentInfo.gpuData" :key="index">
            ...
            >
                <el-popover placement="bottom" :width="250" trigger="hover" v-if="chartsOption[`gpu${index}`]">
                    ...
                    <el-row :gutter="5">
                        <el-tag class="tagClass">{{ $t('monitor.gpuUtil') }}: {{ item.util }}</el-tag>
                    </el-row>
                    <el-row :gutter="5">
                        <el-tag class="tagClass">
                            {{ $t('monitor.temperature') }}: {{ item.temp.replaceAll('C', '°C') }}
                        </el-tag>
                    </el-row>
                    <el-row :gutter="5">
                        <el-tag class="tagClass">{{ $t('monitor.performanceState') }}: {{ item.perf }}</el-tag>
                    </el-row>
                    <el-row :gutter="5">
    ...
    nextTick(() => {
        ...
        for (let i = 0; i < currentInfo.value.gpuData.length; i++) {
            chartsOption.value['gpu' + i] = {
                ...
                data: formatNumber(Number(currentInfo.value.gpuData[i].util.replaceAll(' %', ''))),
            };
        }
