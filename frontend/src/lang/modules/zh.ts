...
const message = {
    ...
    xpack: {
        ...
        vm: {
            ...
            dhcpEnd: 'DHCP 结束地址',
            bootOrder: '启动项',
            bootOrderHelper: 'HD 表示优先从磁盘启动，磁盘不可启动时再尝试镜像；CD-ROM 表示优先从镜像启动。',
            ...
            isoTargetDirOptional: '未填写镜像目标路径，将直接使用镜像源文件路径。',
            isoTargetDirAvailable: '镜像目标路径可用。',
            isoTargetDirUnavailable: '镜像目标路径必须位于可用的虚拟机存储池内。',
            isoTargetDirCheckRequired: '请先检查镜像目标路径，检查通过后再保存。',
            checkPath: '检查',
            isoSize: '镜像大小',
