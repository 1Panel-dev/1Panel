...
const message = {
    ...
    firewall: {
        ...
        resetDirectRulesHelper: '删除 {0} 中全部 1Panel 系统防火墙链、规则、持久化文件及对应数据库记录',
        resetWhitelistRulesHelper:
            '删除 {0} 中全部自定义防火墙配置及对应数据库记录，恢复安装默认状态并停用 {0}。\n清理后将不再提供防火墙防护，此操作不可恢复。',
        ...
        cleanupAction: '清理',
        backendSwitchNotice:
            '系统防火墙切换不会自动迁移或清理规则；端口转发和 Docker 防护必须先清理当前后端才能切换，面板中保存的规则和策略不会被删除，切换后可重新初始化或同步。',
