...
const message = {
    ...
    xpack: {
        ...
        vm: {
            ...
            isoSource: 'ISO 来源',
            isoTargetDir: 'ISO 目标路径',
            isoTargetDirHelper: '填写后 ISO 会保存到该目录；服务器选择可留空。',
            isoTargetDirOptional: '未填写 ISO 目标路径，将直接使用 ISO 源文件路径。',
            isoTargetDirAvailable: 'ISO 目标路径可用。',
            isoTargetDirUnavailable: 'ISO 目标路径必须位于可用的虚拟机存储池内。',
            isoTargetDirCheckRequired: '请先检查 ISO 目标路径，检查通过后再保存。',
            checkPath: '检查',
            isoSize: 'ISO 大小',
            deleteIsoConfirm: '删除操作不可回滚，是否继续？',
            deleteSourceFile: '删除源文件',
            deleteSourceFileHelper: '勾选后会同时删除该 ISO 记录引用的源文件，文件删除后无法恢复。',
            dir: '本地目录',
        },
        cluster: {
            cluster: '应用高可用',
