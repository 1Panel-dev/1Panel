...
const message = {
    ...
    xpack: {
        ...
        vm: {
            ...
            isoSource: 'ISO 來源',
            isoTargetDir: 'ISO 目標路徑',
            isoTargetDirHelper: '填寫後 ISO 會儲存到該目錄；伺服器選擇可留空。',
            isoTargetDirOptional: '未填寫 ISO 目標路徑，將直接使用 ISO 來源檔案路徑。',
            isoTargetDirAvailable: 'ISO 目標路徑可用。',
            isoTargetDirUnavailable: 'ISO 目標路徑必須位於可用的虛擬機儲存池內。',
            isoTargetDirCheckRequired: '請先檢查 ISO 目標路徑，檢查通過後再儲存。',
            checkPath: '檢查',
            isoSize: 'ISO 大小',
            deleteIsoConfirm: '刪除操作不可回滾，是否繼續？',
            deleteSourceFile: '刪除源檔案',
            deleteSourceFileHelper: '勾選後會同時刪除該 ISO 記錄引用的源檔案，檔案刪除後無法恢復。',
            dir: '本機目錄',
        },
        cluster: {
            cluster: '應用高可用',
