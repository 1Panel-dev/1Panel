...
const message = {
    ...
    aiTools: {
        ...
        tensorRT: {
            llm: 'TensorRT LLM',
            modelDir: '模型目錄',
            commandHelper: '若需外部存取，請將命令中的端口設置為與應用端口相同',
    ...
    container: {
        ...
        mirrorsHelper2: '配置請參考官方文件',
        registries: '私有倉庫',
        ipv6Helper: '開啟 IPv6 後，需要增加 IPv6 的容器網路，配置請參考官方文件',
    ...
    cronjob: {
        ...
        taskName: '任務名稱',
        cronSpec: '執行週期',
        cronSpecDoc: '自訂執行週期僅支援【分 時 日 月 週】格式，例如 0 0 * * *，具體參考官方文件。',
    ...
    toolbox: {
        ...
        device: {
            dnsHelper: '伺服器地址域名解析',
            dnsAlert: '請注意修改 /etc/resolv.conf 檔案的配置時，重啟系統後會將檔案復原為預設值',
    ...
    setting: {
        ...
        analysis: '解析',
        analysisHelper: '貼上整個 token 內容，自動解析所需部分，具體操作參考官方文件',
        serviceName: '服務名稱',
        operator: '操作員',
        ...
        code: '授權碼',
        codeHelper:
            '請點擊取得按鈕，然後登入 {0} 複製跳轉連結中 code 後面的內容，貼到該輸入框中，具體操作參考官方文件。',
        googleHelper: '請先建立Google應用並取得用戶端資訊，填寫表單後點擊取得按鈕，具體操作參考官方文件。',
        loadCode: '取得',
        COS: '騰訊雲 COS',
        ap_beijing_1: '北京一區',
        ap_beijing: '北京',
        ...
        passkeyFailed: '通行金鑰註冊失敗，請確認面板SSL證書是否為可信證書',
        bindInfo: '監聽地址',
        bindAll: '監聽所有',
    ...
    app: {
        ...
        showLocal: '顯示本機應用程式',
        reload: '重載',
        upgradeWarn: '升級應用程式會取代 docker-compose.yml 檔案，如有更改，可以點擊查看檔案對比',
        newVersion: '新版本',
        oldVersion: '當前版本',
        composeDiff: '檔案對比',
        showDiff: '看對比',
        ...
        webUIConfig: '當前節點尚未配置預設存取地址，請在應用參數或前往面板設定進行設定',
        toLink: '連結',
    ...
    website: {
        ...
        sni: '回源 SNI',
        sniHelper: '反代後端為 https 的時候可能需要設定回源 SNI，具體需要看 CDN 服務商檔案',
        huaweicloud: '華為雲',
        createDb: '建立資料庫',
    ...
    xpack: {
        ...
        tamper: {
            ...
            protectTemplateHelper:
                '請輸入保護內容，Enter或空格分隔。（具體檔案 ./index.html、檔案後綴 .html、檔案類型 js，保護目錄需要輸入具體目錄 ./log）',
            ...
            protect: '保護',
            protectHelper:
                '請選擇或輸入保護內容，Enter或空格分隔。（具體檔案 ./index.html、檔案後綴 .html、檔案類型 js，保護目錄需要輸入或選擇具體目錄 ./log）',
        ...
        helper: {
            ...
            appTitle3: '即時異常監控',
            appContent3: '行動端即時查看伺服器狀態、WAF 安全監控、網站存取統計與行程健康狀況。',
        ...
        node: {
            ...
            coreExist:
                '目前節點已作為主節點啟用，無法直接作為從節點新增。請先將其降級為從節點後再新增，具體可參考檔案。',
