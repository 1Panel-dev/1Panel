...
const message = {
    commons: {
        ...
        msg: {
            ...
            fileNameErr: '僅支援上傳名稱包含英文、中文、數字或 .-_ ,長度 1-256 位的文件',
    ...
    database: {
        ...
        clickHelper: '點擊上傳',
        supportUpType:
            '僅支援 sql、sql.gz、tar.gz、.zip 檔案格式，匯入的壓縮文件必須保證只有一個 .sql 文件或包含 test.sql',
    ...
    container: {
        ...
        composeHelper: '通過 1Panel 編輯或模版建立的編排，將儲存在 {0}/docker/compose 路徑下',
        deleteFile: '刪除檔案',
    ...
    cronjob: {
        ...
        database: '備份資料庫',
        backupArgs: '備份參數',
        backupArgsHelper: '未列出的備份參數可手動輸入並選擇，例如：輸入 --no-data 後選擇下拉列表中的第一個選項',
        singleTransaction: '使用單一事務備份 InnoDB 表，適用於大資料量的備份',
        quick: '逐行讀取資料，而不是將整個表加載到內存中，適用於大資料量和低內存機器的備份',
    ...
    setting: {
        ...
        reCreate: '建立快照失敗',
        reRollback: '回滾快照失敗',
        deleteHelper: '將刪除該快照的所有備份檔案，包括第三方備份帳號中的文件。',
        ignoreRule: '排除規則',
        ...
        bindDomainHelper: '設定網域綁定後，僅能透過設定中域名訪問 1Panel 服務',
        bindDomainHelper1: '綁定域名為空時，則取消網域綁定',
        bindDomainWarning: '設定網域綁定後，將退出目前登入，且僅能透過設定中域名訪問 1Panel 服務，是否繼續？',
        allowIPs: '授權 IP',
        unAllowIPs: '取消授權',
    ...
    app: {
        ...
        showLocal: '顯示本機應用程式',
        reload: '重載',
        upgradeWarn: '升級應用程式會取代 docker-compose.yml 文件，如有更改，可以點擊查看檔案對比',
        newVersion: '新版本',
        oldVersion: '目前版本',
        composeDiff: '檔案對比',
        showDiff: '看對比',
        ...
        webUIConfig: '目前節點尚未配置預設存取地址，請在應用參數或前往面板設定進行設定！',
        toLink: '連結',
    ...
    website: {
        ...
        failTimeout: '故障超時',
        failTimeoutHelper:
            '服務器健康檢查的時間窗口長度。在該時間段內累計失敗次數達到閾值時，服務器將被暫時移除，並在經過相同時長後重新嘗試。預設 10 秒',
    ...
    nginx: {
        ...
        sslRejectHandshakeHelper: '開啟之後可以避免證書洩露，設置預設網站會讓此設置失效',
    },
    ssl: {
        create: '申請證書',
        provider: '類型',
    ...
    runtime: {
        ...
        goDirHelper: '目錄中要包含 go 文件或二進制文件，子目錄中包含也可',
        extension: '擴充',
    ...
    xpack: {
        ...
        tamper: {
            ...
            ignoreTemplateHelper:
                '請輸入排除內容，Enter或空格分隔。（具體目錄 ./log 或目錄名 tmp，排除檔案需要輸入具體檔案 ./data/test.html）',
            protectTemplateHelper:
                '請輸入保護內容，Enter或空格分隔。（具體檔案 ./index.html、文件後綴 .html、檔案類型 js，保護目錄需要輸入具體目錄 ./log）',
            ...
            ignore: '排除',
            ignoreHelper:
                '請選擇或輸入排除內容，Enter或空格分隔。（具體目錄 ./log 或目錄名 tmp，排除檔案需要輸入或選擇具體檔案 ./data/test.html）',
            protect: '保護',
            protectHelper:
                '請選擇或輸入保護內容，Enter或空格分隔。（具體檔案 ./index.html、文件後綴 .html、檔案類型 js，保護目錄需要輸入或選擇具體目錄 ./log）',
            ...
            appendOnly: '僅追加',
            appendOnlyHelper: '將限制該目錄下文件的刪除操作，僅允許新增排除目錄或非保護檔案',
            immutable: '不可變',
        ...
        customApp: {
            ...
            nodeHelper: '不能選擇當前節點',
            migrateHelper: '目前僅支援遷移單體應用和只關聯 MySQL、MariaDB、PostgreSQL 資料庫的應用',
        ...
        alert: {
            ...
            clamsRule: '病毒掃描任務檢測到感染檔案告警，每天發送{0}次',
