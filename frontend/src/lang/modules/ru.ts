...
const message = {
    ...
    xpack: {
        ...
        user: {
            ...
            auth: {
                ...
                saml2: {
                    ...
                    usedAttribute: 'Используемый атрибут имени пользователя',
                    usersAndRoles: 'Пользователи и роли',
                    usersAndRolesDesc:
                        'Сопоставляется только имя пользователя. Пользователь 1Panel автоматически создается при первом успешном входе.',
                },
                ldap: {
                    basic: 'Основные настройки',
                    enable: 'Включить',
        ...
        node: {
            ...
            healthCheckFrequency: 'Частота проверки',
            healthCheckFrequencyHelper:
                'Периодически проверяет состояние серверных узлов. Введите 0, чтобы отключить проверки.',
            healthCheckFrequencyLimit:
                'Частота проверки не может превышать один день и должна быть неотрицательным целым числом.',
            loadFailedNodeResources: 'Загружать ресурсы недоступных узлов',
            loadFailedNodeResourcesHelper:
                'При открытии списка узлов также загружать сведения о ресурсах недоступных узлов.',
