...
const message = {
    apiKeyManagement: {
        ...
        bindingDisableHelp:
            'Отключение привязки по QR-коду запрещает новые привязки. Уже привязанные устройства могут продолжать работу, пока ключ не будет отключён, отозван или не истечёт срок его действия.',
        saveSecret:
        ...
        legacyHelp:
            'Прежний ключ обеспечивает совместимость с ранее настроенными API-подключениями. Включение, отключение и сброс действуют только на этот ключ.',
    ...
    setting: {
        ...
        apiKeyValidityTimeHelper:
            'Срок действия подписи запроса в минутах, отсчитываемый от метки времени запроса. Значение 0 отключает проверку метки времени.',
    ...
    xpack: {
        ...
        alert: {
            ...
            alertTriggerMode: 'Условие уведомления',
            alertTriggerFailed: 'Только при ошибке',
            alertTriggerSuccess: 'Только при успехе',
            alertTriggerBoth: 'При успехе или ошибке',
            alertNotificationHelper:
                'Уведомление отправляется по окончательному результату, а не во время повторных попыток. Уведомления об успехе и ошибке используют общий дневной лимит и часы отправки.',
            notificationTitle: 'Плановая задача - {0} «{1}»: уведомление о выполнении',
            notificationSuccessTitle: 'Плановая задача - {0} «{1}» успешно завершена',
            notificationRule: '{0}: {1}, до {2} уведомлений в день',
