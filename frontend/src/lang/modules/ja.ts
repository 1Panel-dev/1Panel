...
const message = {
    ...
    xpack: {
        ...
        vm: {
            ...
            isoTargetDirHelper: '設定すると ISO はこのディレクトリに保存されます。サーバー選択の場合は空にできます。',
            isoTargetDirOptional: 'ISO ターゲットパスが設定されていないため、元の ISO ソースファイルパスを使用します。',
            isoTargetDirAvailable: 'ISO ターゲットパスは利用可能です。',
            isoTargetDirUnavailable: 'ISO ターゲットパスは利用可能な仮想マシンストレージプール内である必要があります。',
            isoTargetDirCheckRequired: '保存前に ISO ターゲットパスを確認してください。',
            checkPath: '確認',
            isoSize: 'ISO サイズ',
