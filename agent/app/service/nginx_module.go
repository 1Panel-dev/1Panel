...
var (
	nginxModulePackagePattern  = regexp.MustCompile("^[a-zA-Z0-9][a-zA-Z0-9+.-]*$")
	nginxModuleArtifactPattern = regexp.MustCompile(`^[a-zA-Z0-9_./+-]+\.so$`)
	nginxModuleChecksumPattern = regexp.MustCompile(`^[a-fA-F0-9]{64}$`)

	errNginxModuleBuilderMissing = errors.New("dynamic module builder not found")
)
...
func loadNginxModules(install model.AppInstall) ([]dto.NginxModule, error) {
	modulePath := path.Join(install.GetPath(), "build", "module.json")
	modules, err := readNginxModuleFile(modulePath)
	if err != nil {
		return nil, err
	}
	catalogPath := path.Join(install.GetPath(), "build", "module.catalog.json")
...
func resolveNginxModuleTarget(install model.AppInstall) (dto.NginxModuleTarget, string, error) {
	builderPath := path.Join(install.GetPath(), "build", "Dockerfile.modules")
...
func shortNginxModuleHash(value string) string {
	if len(value) <= 12 {
		return value
	}
	return value[:12]
...
func validateNginxModulePackages(packages []string) error {
	for _, item := range packages {
		if !nginxModulePackagePattern.MatchString(item) {
...
func (localNginxModuleProvider) Resolve(spec nginxModuleBuildSpec) (dto.NginxModuleBuild, error) {
	...
	modulesRoot := path.Join(spec.Install.GetPath(), "modules")
	if !spec.Force {
	...
	buildArgs := []string{
		...
		"-f", path.Join(spec.BuildPath, "Dockerfile.modules"),
		"-t", tempImage,
	...
	stagingPath := path.Join(spec.Install.GetPath(), "modules", ".staging", modulePathName+"-"+shortHash)
	...
	_ = os.WriteFile(path.Join(finalPath, "manifest.json"), manifest, constant.FilePerm)
	buildComplete = true
	return result, nil
}
...
func collectNginxModuleArtifacts(root, relativeRoot string) ([]dto.NginxModuleArtifact, error) {
	...
	err := filepath.Walk(root, func(filePath string, info os.FileInfo, walkErr error) error {
		...
		if info.IsDir() || filepath.Ext(info.Name()) != ".so" || filepath.Base(filepath.Dir(filePath)) == "lib" {
			return nil
		}
...
func resolveNginxModuleArtifactPath(install model.AppInstall, artifactPath string) (string, error) {
	if artifactPath == "" || path.IsAbs(artifactPath) || path.Clean(artifactPath) != artifactPath ||
		strings.Contains(artifactPath, `\`) || strings.HasPrefix(artifactPath, "../") ||
		!nginxModuleArtifactPattern.MatchString(artifactPath) {
		return "", fmt.Errorf("invalid module artifact path %q", artifactPath)
	}
	modulesRoot := filepath.Clean(path.Join(install.GetPath(), "modules"))
	artifactFullPath := filepath.Clean(filepath.Join(modulesRoot, filepath.FromSlash(artifactPath)))
	rel, err := filepath.Rel(modulesRoot, artifactFullPath)
	if err != nil || rel == ".." || strings.HasPrefix(rel, ".."+string(os.PathSeparator)) {
		return "", fmt.Errorf("module artifact path %q escapes the module directory", artifactPath)
	}
...
func validateNginxModuleArtifacts(install model.AppInstall, artifacts []dto.NginxModuleArtifact) error {
	...
	for _, artifact := range artifacts {
		...
		if !nginxModuleChecksumPattern.MatchString(artifact.Checksum) {
			return fmt.Errorf("invalid checksum for module artifact %q", artifact.Path)
		}
		artifactFullPath, err := resolveNginxModuleArtifactPath(install, artifact.Path)
		if err != nil {
			return err
		}
...
// nginxModuleBuildFailedError identifies the module a dynamic build failed
// for, so callers can decide whether a static fallback is allowed.
type nginxModuleBuildFailedError struct {
	Module string
	Err    error
}

func (e *nginxModuleBuildFailedError) Error() string {
	return e.Err.Error()
}

func (e *nginxModuleBuildFailedError) Unwrap() error {
	return e.Err
}
...
// shouldFallbackNginxModuleToStatic reports whether a failed dynamic build may
// fall back to a static build. Only auto mode gets the fallback; an explicit
// dynamic choice keeps the failure semantics.
func shouldFallbackNginxModuleToStatic(module dto.NginxModule) bool {
	normalizeNginxModule(&module)
	return module.BuildMode == nginxModuleBuildAuto
}

// fallbackNginxModuleToStatic flips the module identified by buildFailed to a
// static build after its dynamic build failed. It reports whether the fallback
// was performed.
func fallbackNginxModuleToStatic(modules []dto.NginxModule, buildFailed *nginxModuleBuildFailedError) bool {
	for i := range modules {
		if modules[i].Name != buildFailed.Module {
			continue
		}
		if !shouldFallbackNginxModuleToStatic(modules[i]) {
			return false
		}
		modules[i].BuildMode = nginxModuleBuildStatic
		modules[i].LastError = fmt.Sprintf("dynamic build failed: %v; module switched to static build", buildFailed.Err)
		return true
	}
	return false
}
...
func buildDynamicNginxModules(install model.AppInstall, modules []dto.NginxModule, selected []string, force bool, parentTask *task.Task) ([]dto.NginxModule, error) {
	...
	buildPath := path.Join(install.GetPath(), "build")
	for i := range modules {
		module := &modules[i]
		normalizeNginxModule(module)
		if !nginxModuleNeedsDynamicBuild(*module, selectedNames) {
		...
		if providerErr != nil {
			return modules, &nginxModuleBuildFailedError{Module: module.Name, Err: providerErr}
		}
		...
		if buildErr != nil {
			...
			return failedModules, &nginxModuleBuildFailedError{Module: module.Name, Err: fmt.Errorf("build dynamic module %s: %w", module.Name, buildErr)}
		}
		module.DynamicSupport = nginxModuleSupportSupported
		err = validateNginxModuleArtifacts(install, build.Artifacts)
		if err != nil {
			removeNginxModuleBuildOutput(install, build)
			build.Status = nginxModuleStatusFailed
			...
			removeNginxModuleOutputsNotReferenced(install, modules, failedModules)
			_ = saveNginxModules(install, failedModules)
			return failedModules, &nginxModuleBuildFailedError{Module: module.Name, Err: fmt.Errorf("validate dynamic module %s: %w", module.Name, err)}
		}
		module.LastError = ""
...
func removeNginxModuleBuildOutput(install model.AppInstall, build dto.NginxModuleBuild) {
...
func validateNginxModuleLoadConfig(install model.AppInstall, target dto.NginxModuleTarget, validationName, loadDirectives string) error {
	...
	testConfig := path.Join(install.GetPath(), "modules", ".test-"+nginxModulePathName(validationName)+"-"+hex.EncodeToString(configSum[:6])+".conf")
	if err := os.WriteFile(testConfig, []byte(config.String()), constant.FilePerm); err != nil {
		return err
	}
	defer os.Remove(testConfig)
	modulesRoot := path.Join(install.GetPath(), "modules")
	args := []string{
		"run", "--rm", "--network", "none",
...
func configureStaticNginxModules(install model.AppInstall, modules []dto.NginxModule, mirror string) error {
	...
	preScriptPath := path.Join(buildPath, "tmp", "pre.sh")
...
func reconcileDynamicNginxModuleConfig(install model.AppInstall, modules []dto.NginxModule, reload bool) error {
	...
	configDir := path.Join(install.GetPath(), "conf", "modules-enabled")
	...
	if err = applyManagedNginxModuleConfigs(configDir, desired); err != nil {
		_ = restoreManagedNginxModuleConfigs(configDir, snapshot)
		return err
	}
	if !reload {
		return nil
...
func restoreManagedNginxModuleConfigs(configDir string, snapshot nginxModuleConfigSnapshot) error {
	return applyManagedNginxModuleConfigs(configDir, snapshot)
}
...
func executeNginxModuleBuild(install model.AppInstall, reqModules []string, force bool, mirror string, parentTask *task.Task, reload bool) error {
	...
	if !staticBuild && hasDynamicNginxModuleBuildTask(modules, reqModules) {
		...
		// ships no dynamic builder; the automatic flows degrade instead.
		builderPath := path.Join(install.GetPath(), "build", "Dockerfile.modules")
		if _, statErr := os.Stat(builderPath); statErr != nil {
			return fmt.Errorf("%w: %v", errNginxModuleBuilderMissing, statErr)
		}
	}
	if staticBuild {
	...
	if err != nil {
		...
		if !errors.As(err, &buildFailed) || !fallbackNginxModuleToStatic(modules, buildFailed) {
			return err
		}
		// Auto mode promises a working module, not a build strategy: the failed
		// module was flipped to static, now run the full static chain.
		parentTask.Logf("WARNING: dynamic build of module %s failed: %v; falling back to static build", buildFailed.Module, buildFailed.Err)
		if staticErr := executeStaticNginxModuleBuild(install, modules, mirror, force, parentTask); staticErr != nil {
			for i := range modules {
				if modules[i].Name == buildFailed.Module {
					modules[i].BuildMode = nginxModuleBuildAuto
					modules[i].LastError = fmt.Sprintf("dynamic build failed: %v; static fallback also failed: %v", buildFailed.Err, staticErr)
					break
				}
			}
			_ = saveNginxModules(install, modules)
			return fmt.Errorf("%v; static fallback build also failed: %w", err, staticErr)
		}
		return nil
	}
...
func removeNginxModuleArtifacts(install model.AppInstall, module dto.NginxModule) error {
	for _, build := range module.Builds {
		moduleDir := path.Join(install.GetPath(), "modules", build.Target.Key, nginxModulePathName(module.Name))
