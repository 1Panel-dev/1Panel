...
func (f *fakeLegacyBackend) UnbindChain(table, parentChain, chain string) error {
	f.calls = append(f.calls, backendCall{method: "unbind", table: table, args: []string{parentChain, chain}})
	return f.err
}

func (f *fakeLegacyBackend) ClearChain(table, chain string) error {
	f.calls = append(f.calls, backendCall{method: "clear", table: table, args: []string{chain}})
	return f.err
}
...
func (f *fakeForwardingSystem) Remove(name string) error {
...
func TestLegacyListParsingAndOwnershipContract(t *testing.T) {
	...
	for _, foreign := range []struct{ table, chain string }{
		{iptables.NatTab, "PREROUTING"},
		{iptables.NatTab, "DOCKER"},
		{iptables.NatTab, "THIRD_PARTY_DNAT"},
		{iptables.FilterTab, "FORWARD"},
		{iptables.FilterTab, "DOCKER-USER"},
	} {
		if IsOwnedChain(foreign.table, foreign.chain) {
			t.Fatalf("foreign chain selected for cleanup: %s/%s", foreign.table, foreign.chain)
		}
	}
	if !IsOwnedChain(iptables.NatTab, ChainPreRouting) || !IsOwnedChain(iptables.NatTab, ChainPostRouting) ||
		!IsOwnedChain(iptables.FilterTab, ChainForward) {
		t.Fatal("forwarding-owned chains must be recognized")
	}
}

func TestLegacyCleanupOnlyTouchesOwnedObjects(t *testing.T) {
	backend := &fakeLegacyBackend{stdout: map[string]string{}}
	system := &fakeForwardingSystem{}
	adapter := &legacyNATAdapter{provider: "iptables", backend: backend, system: system}
	if err := adapter.CleanupOwned(); err != nil {
		t.Fatal(err)
	}
	for _, call := range backend.calls {
		chain := call.args[len(call.args)-1]
		if call.method == "unbind" {
			chain = call.args[1]
		}
		if !IsOwnedChain(call.table, chain) {
			t.Fatalf("cleanup touched foreign chain: %#v", call)
		}
	}
	if len(system.removed) != 3 {
		t.Fatalf("cleanup removed unexpected persistence files: %#v", system.removed)
	}
	for _, file := range system.removed {
		if !strings.HasSuffix(file, ForwardFile) && !strings.HasSuffix(file, PreRoutingFile) && !strings.HasSuffix(file, PostRoutingFile) {
			t.Fatalf("cleanup removed foreign file: %s", file)
		}
	}
}
