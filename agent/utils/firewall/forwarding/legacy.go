...
type legacyIptablesBackend interface {
	...
	UnbindChain(table, parentChain, chain string) error
...
func (systemIptablesBackend) UnbindChain(table, parentChain, chain string) error {
	return iptables.UnbindChain(table, parentChain, chain)
}

func (systemIptablesBackend) ClearChain(table, chain string) error {
	return iptables.ClearChain(table, chain)
}
...
func (defaultForwardingSystem) Remove(name string) error {
...
func (l *legacyNATAdapter) CleanupOwned() error {
	var result error
	for _, item := range []struct {
		table  string
		parent string
		...
		result = errors.Join(result, l.backend.UnbindChain(item.table, item.parent, item.chain))
		result = errors.Join(result, l.backend.ClearChain(item.table, item.chain))
		result = errors.Join(result, l.backend.Run(item.table, "-X", item.chain))
	}
	for _, file := range []string{ForwardFile, PreRoutingFile, PostRoutingFile} {
		if err := l.system.Remove(path.Join(global.Dir.FirewallDir, file)); err != nil && !os.IsNotExist(err) {
